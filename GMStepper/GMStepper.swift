...
@IBDesignable public class GMStepper: UIControl {
	...
	/// Disable the actions sent when a new value has been set. One time only
	...
	@objc @IBInspectable public var value: Double = 0 {
		didSet {
			...
			if isInteger && stepValue == 1.0 && items.count > 0 {
				...
				label.text = value.toString(fractionDigits: 2)
			}

			if oldValue != value, !disableActions {
	...
	*/
	...
	lazy var label: UILabel = {
		...
		if self.showIntegerIfDoubleIsInteger && floor(self.value) == self.value {
			...
			label.text = self.value.toString(fractionDigits: 2)
		}
...
private extension Double {
	...
	func toString(fractionDigits: Int, numberStyle: NumberFormatter.Style = .decimal) -> String {
